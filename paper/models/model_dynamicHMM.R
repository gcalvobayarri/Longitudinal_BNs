...
code2 <- nimbleCode({
  ...
  for (i in 1:N) {
    for (j in 1:T) {
      ...
      logit(p1[i, j]) <-  beta0[1, Z[j]] + betaSG[1]*SG[i, j] + betaPG[1]*PG[i, j] +
        betaSF[1]*SF[i, j] + betaPF[1]*PF[i, j] +
        betaH[1]*home[i, j] + b0[i, 1]
      
      logit(p2[i, j]) <-  beta0[2, Z[j]] + betaSG[2]*SG[i, j] + betaPG[2]*PG[i, j] +
        betaSF[2]*SF[i, j] + betaPF[2]*PF[i, j] +
        betaH[2]*home[i, j] + b0[i, 2]
      
      logit(p3[i, j]) <-  beta0[3, Z[j]] + betaSG[3]*SG[i, j] + betaPG[3]*PG[i, j] +
  ...
  for (k in 1:3) {
    ...
    beta0.p[k, 1]   ~ dnorm(0, 0.1) #new parameters 
    beta0.p[k, 2]   ~ dnorm(0, 0.1)
    
    beta0[k, 1] <- min(beta0.p[k, ])
    beta0[k, 2] <- max(beta0.p[k, ]) # constrains
  ...
  p0[1] ~ dbeta(1, 1) # new parameter
  p0[2] <- 1 - p0[1]
  
  for (j in 2: T){
    Z[j] ~ dcat(omega[Z[j-1], ])
  }
...
inits <- list( beta0.p=matrix(data = c(0.45, 0.45, -3.5, 0.5, 0.5, -3), nrow = 3, 
                              ncol = 2), 
               betaH=c(0, 0, 0), alpha0=c(0.5, 0.5, -5),
               alpha=c(0, 0, 0), gamma0=-1, gamma=0, delta0 = 3, sigmab=c(1, 1, 1),
               sigmaa=c(1, 1, 1), sigmac=1, sigmad=1, pCH = .5, pHC = .5, p0=c(.5,.5),
               Z= sample(1:2, size = 82, replace = TRUE))

HMModel_samples <- nimbleMCMC(code = code2, 
                                  constants = data_model,
                                  inits = inits,
                                  nburnin = 500000, niter = 1000000,
                                  summary = T, thin = 500,
                                  monitors = parameters,
                                  dimensions = list(omega = c(2, 2)),
                                  nchains = 3,
                                  WAIC = T)

HMModel_samples$summary$all.chains 

save(HMModel_samples, file = 'paper/results/HMModel_samples.RData')

HMModel_samples$WAIC$WAIC
